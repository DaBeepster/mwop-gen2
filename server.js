...
rl.on("line", function(d) {
  ...
  if (msg.startsWith("/")) {
    ...
    if (cmdCheck[0] == "help") {
      ...
      console.log("/rank <user|moderator|admin|owner|server|tell|discord> - Changes your rank. (Only affects messages.)");
      ...
      var rankIndex = ["user", "moderator", "admin", "owner", "server", "tell", "discord"].indexOf(cmdCheck[1].toLowerCase())
      if (~rankIndex) {
        serverOpRank = rankIndex;
        console.log("Set rank to " + cmdCheck[1].toLowerCase() + ".");
      } else {
        console.log("Usage: /rank <user|moderator|admin|owner|server|tell|discord>")
      }
    }
  } else {
    ...
    sendToWorlds((serverOpNick && ["[0] ", "", " ", "[Server] "][serverOpRank] || ["", "(M) ", "(A) ", "(O)", "Server", "-> ", "[D] "][serverOpRank]).trimLeft() + (serverOpNick || (serverOpRank == 4 ? "" : "0")) + ": " + msg);
  }
});
beginServer()
